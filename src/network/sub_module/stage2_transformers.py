...
class TransformerDecoder(nn.Module):
    def __init__(self, num_classes, d_model, nhead, num_layers = 4, dim_feedforward = 512, device = 'cpu', input_dim = None, args = None):
        ...
        decoder_layer = nn.TransformerDecoderLayer(d_model, nhead, dim_feedforward, device = device, dropout = args.dropout)
    ...
    def forward(self, x):
        ...
        '''
        # print('x.shape:', x.shape) #torch.Size([bs, 60, 261])
...
class TransformerFull(nn.Module):
    def __init__(self, num_classes, d_model, nhead = 3, num_layers = 3, device = 'cpu', input_dim = None, args = None):
        ...
        self.transformer = nn.Transformer(d_model=d_model, nhead=nhead, num_encoder_layers=num_layers, 
                                          num_decoder_layers=num_layers, dropout=args.dropout)
...
class TransformerEncoderCls(nn.Module):
    def __init__(self, num_classes, d_model, nhead = 3, num_layers = 4, device = 'cpu', input_dim = None, args = None):
        super(TransformerEncoderCls, self).__init__()
        self.input_dim = input_dim
        if not input_dim is None:
            self.input_projection = nn.Linear(input_dim, d_model) if input_dim != d_model else nn.Identity()
        self.pos_encoder = PositionalEncoding(d_model, device = device)
        encoder_layer = nn.TransformerEncoderLayer(d_model, nhead, d_model * 4, dropout = args.dropout)
